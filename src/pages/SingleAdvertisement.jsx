        ...
        <div className='search-container'>
            ...
            <BottomDrawer isOpen={open} onClose={() => setOpen(false)} handleSelect={handleSelect}>
                <div className='not-partner'>
                    ...
                    <a href={`https://wa.me/${item.phone.split('(').join('').split(')').join('').split('-').join('')}?text=${encodeURIComponent('Здравствуйте! Мы хотели забронировать у вас домик через https://booklink.pro/, но не получилось, так как оказалось, что вы не зарегистрированы в этом сервисе. Поэтому пишем вам в WhatsApp. Можем уточнить информацию о свободных датах и номерах?')}`} className='whatsapp-btn'>
                        Написать в WhatsApp
                    </a>
